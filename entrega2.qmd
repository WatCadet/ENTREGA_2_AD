...
Como hay 30 observaciones utilizamos la prueva de Shapiro-Wilk.

Hacemos el test para cada nivel:
```{r,output=TRUE}
shapiro.test(as.numeric(data$alcohol=="No"))
```
```{r}
shapiro.test(as.numeric(data$alcohol=="Ocasionalment"))

```
```{r}
shapiro.test(as.numeric(data$alcohol=="Setmanalment"))

```
```{r}
shapiro.test(as.numeric(data$alcohol=="Diariament"))

```
Vemos que ninguno de los niveles sigue un distrubución normal (los p-valores son menores a 0.05 según el test) no podemos aplicar una ANOVA. Entonces tendremos que utilizar un test no paramétrico:
...
Hacemos el test para cada nivel:
```{r,output=TRUE}
shapiro.test(as.numeric(data$bebida_gas=="Mai"))
```
```{r,output=TRUE}
shapiro.test(as.numeric(data$bebida_gas=="Menys de dos cops per setmana"))
```
```{r,output=TRUE}
shapiro.test(as.numeric(data$bebida_gas=="Ocasionalment"))
```
```{r,output=TRUE}
shapiro.test(as.numeric(data$bebida_gas=="Diàriament"))
```
Vemos que ninguno de los niveles sigue un distrubución normal (los p-valores son menores a 0.05 según el test) no podemos aplicar una ANOVA. Entonces tendremos que utilizar un test no paramétrico:
Utilizaremos el test de Kruskal
```{r}
